...
<div class="container x-mt-2">
    <input class="searchButton" type="text" placeholder="Buscar" v-model="nombre" @input="nombre = $event.target.value.toUpperCase()">
    <div v-for="stem of searchStems" :key="stem">
        <section class="stem" :id="stem.id">
            <h1 class="xf-2 xf-b upper" :id="stem.id">{{stem.name}}</h1>
            <div class="xf-4 xf-gris">{{stem.genre}} | {{stem.bpm}} | {{stem.key}}
              <br><span class="xf-3">Artista: {{stem.artist}}</span>
            </div><br>
            <span class="art-bio">
            <div v-html="stem.descripcion">
            </div></span>
            <br>
            <a class="stems-btn" :href="stem.enlace" target="_blank">Descargar Stems</a>
            <br><br>
        </section>
    </div>
</div>
...
var urlArtistas = 'https://raw.githubusercontent.com/xpedn/xpedn.github.io/master/dt/stems.json'
export default {
  name: 'Stems',
  created: function(){
    this.getStems();
  ...
  data(){
    return{
      ...
      site: {
          ...
          descripcion: "Encuentra algunos Stems de parte de los artistas de Xpend Music"
        }
    }
  },
  methods: {
  ...
  computed: {
      searchStems: function () {
          return this.stems.filter((item) => item.name.toUpperCase().includes(this.nombre));
      }
  }   
}
</script>
