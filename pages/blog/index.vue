<template>
  <div class="container x-mt-2">
    <div v-for="article of articles.slice(0, 1)" :key="article.slug" class="grid md:grid-2 gap-10">
        <div class="center-y">
          <div>
            ...
            <p class="blog-act">{{ $moment(article.createdAt).format('l')}} • {{ $moment(article.createdAt).fromNow()}}</p>
            <p><nuxt-link :to="'/blog/autor/' + article.author.name" class="autor-blg">{{ article.author.name }}</nuxt-link></p>
          </div>
        </div>
        <div class="">
          <NuxtLink :to="{ name: 'blog-slug', params: { slug: article.slug } }">
    ...
    data(){
      return{
        ...
        site: {
            ...
            descripcion: "Un lugar donde podrás encontrar un gran variedad de articulos relacionados con la música."
          }
      }
    },
    async asyncData({ $content, params }) {
      var articles = await $content('articles', params.slug)
